...
def generatePromptMessages(user_input : str,guard_input : str, ai_input: str = ""):
    ...
    # ChatPromptTemplate takes list of messages therefore create the list and only put existing ones
    # Or there is a change that secret has never been being inserted to the prompt
    ...
    print(f"Crafted Message for AI : {chat_prompt.format_prompt(user_input=user_input,guard_input=guard_input,ai_input=ai_input).to_messages()}")
...
def runAgent_EasyMode(user_input:str,system_context:str,sanitizer:str = None,input_guard:str = None):
    """It is used each time input passes from the guard. By default it runs one time, if additional arguments are provided Guard will be iterated and answer will be feed forwarded to the next agent."""
    # Start assiging a fine graded answer in the begining directly to user input
    # 3 times filtration makes it really hard to extract information
    # Also input guard could render user answer completely useless if it is malicious and return it completely different answer
    # Easy mode therefore will keep the user_input as it is but provide additional context stating what has happened before
    fine_grained_input = user_input
    if input_guard:
        # Define Guard role 
        input_guard_role = f"You are an input guard. User sends you an input. Your tasks are following :\n{input_guard}\nWhen you finish processing, you should return fine tuned user input according to the your tasks guidelines"
        fine_grained_input = runGuard(user_input=fine_grained_input,guard_input=input_guard_role).content
        print(f"Input guard : {fine_grained_input}")
        fine_grained_input = f"User has sent this message {user_input}\nHowever Input guard catched suspicious prompt and returned this answer {fine_grained_input} . Please be cautious about what user asks"
    # System Context is a must therefore it needs to be there
    system_context_role = f"You are an helpful AI assistant. Your roles are : {system_context}\nAnswer to user question accordingly."
    system_context_role += fine_grained_input
    AI_Answer = runGuard(user_input=user_input,guard_input=system_context_role).content
    print(f"System guard : {AI_Answer}")
    if sanitizer:
        # Define Sanitizer role
        sanitizer_role = f"You are Sanitizer. You have been provided with user question and AI answer. Your role is to check AI answer according to your tasks and return an answer to the user. Your tasks are : {sanitizer}"
        AI_Answer = runGuard(user_input=user_input,guard_input=sanitizer_role,ai_input=AI_Answer).content
    return AI_Answer
